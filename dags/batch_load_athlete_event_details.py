...
# constants
SOURCE_DIR = Path("/usr/local/airflow/Data/staged/Olympic_Athlete_Event_Details_Files")
ARCHIVE_DIR = Path("/usr/local/airflow/Data/archieve/Olympic_Athlete_Event_Details_Files_archieve")
BATCH_SIZE = 5
POSTGRES_CONN_ID= 'postgres_initial'

with DAG(
    dag_id= "olympic_athlete_event_details_dag",
    ...
    def parse_files(file_paths):
        ...
        def make_list_of_tuples(file_path):
            ...
            # Process each row to handle NaN values
            processed_rows = []
            for row in rows:
                processed_row = [None if (isinstance(x, float) and math.isnan(x)) else x for x in row]
