...
# constants
SOURCE_DIR = Path("/usr/local/airflow/Data/staged/Olympic_Country_Profiles_Files")
ARCHIVE_DIR = Path("/usr/local/airflow/Data/archieve/Olympic_Country_Profiles_Files_archieve")
BATCH_SIZE = 5
POSTGRES_CONN_ID= 'postgres_initial'

with DAG(
    dag_id= "olympic_country_profile_dag",
    ...
    def load_to_db(tuple_data):
        ...
        try:
            ...
            cursor.executemany("""
                INSERT INTO bronze.Olympic_Country_Profiles (
                    noc, country
                ) VALUES (%s, %s);
            """, tuple_data)
